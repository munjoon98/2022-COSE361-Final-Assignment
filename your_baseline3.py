...
#baseline3
#baseline2에서 캡슐을 먹도록 하는 기능이 추가됨, 우리가 적 진영에 있는데 파워캡슐을 먹은 상태면 굳이 적을 피하지 않으려함, 진영으로 돌아가지않음
#우리 pacman이 음식을 먹었을때 본진으로 돌아가는 알고리즘이 개선됨
#defender가 처음에 maze 벽에 갇히지 않게끔 알고리즘 개선(여기저기 돌아다니게)
...
class FirstAgent(DummyAgent):
  ...
  def getFeatures(self, gameState, action):
    ...
    #food를 먹은 경우, 그리고 그것이 파워캡슐이 아닌경우 flag를 1로 설정(파워캡슐이면 무적이므로 본진으로 돌아갈 필요가 없습니다)
    if(len(foodList)!=len(foodList2) and self.flag!=2): 
      self.flag=1
    ...
    if myStatetwo.isPacman:       #내가 공격중이고 상대방도 자기 진영에 있을때 거리가 너무 가까우면 피하는 방식
      ...
      #적들중에 자기 본진에 있는 경우, 적이 겁에 질린경우 피하지 않아도됨
      if len(attackers) > 0:
       ...
       if(mindists<=3) : self.flag=1 #파워캡슐을 얼마전에 먹었는데 강한 적이 근접하다면 무적인 상태가 끝났다고 판단하고 후퇴 
    ...
    if len(capsuleList) > 0: #파워캡슐이 있을경우
      ...
      if(len(capsuleList)!=len(capsuleListNext)): #캡슐을 먹은경우
       features['redistanceToCapsule'] = 12 #해당 feature에 큰 값 부여
       self.flag=2 #flag 2로 설정(무적)

    return features

  def getWeights(self, gameState, action):
    return {'successorScore': 20, 'distanceToFood': -1, 'onOffense':-100000, 'attackerDistance':-1, 'redistanceToCapsule':1, 'isSame' : -100}
