...
#baseline2
#baseline1에서 우리 agent가 상대방 진영에 있고 상대방도 자기 진영에 있을때 거리가 너무 가까우면 피하는 기능이 추가됐습니다
#(거리가 1,2와같이 가까울 경우)
...
class FirstAgent(DummyAgent):
  ...
  def getFeatures(self, gameState, action):
    ...
    myPostwo = myStatetwo.getPosition() #다음상태의 agent의 위치
    if myStatetwo.isPacman:       #내가 공격중이고 상대방도 자기 진영에 있을때 거리가 너무 가까우면 피하는 방식
      ...
      attackers = [a for a in enemies if (not a.isPacman) and a.getPosition() != None]  #적들중에 자기 본진에 있는 경우
      if len(attackers) > 0:  
       ...
       features['attackerDistance'] = 6*(2/float(mindists)) #적과의 거리가 1,2일경우 웬만하면 피하려함

    return features

  def getWeights(self, gameState, action):
    return {'successorScore': 100, 'distanceToFood': -1, 'onOffense':-100000, 'attackerDistance':-1} #feature들의 weight값
